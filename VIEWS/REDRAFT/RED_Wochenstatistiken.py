...
REDRAFTLEAGUES_2025 = REDLEAGUES

# st.set_page_config(page_title="Sleeper Standings 2025", layout="wide")

st.title("📊 Sleeper Standings – Redraft 2025")

# Funktion: Standings abrufen
def get_standings(league_id):
    ...
    for r in rosters:
        ...
        fpts_dec = r.get("settings", {}).get("fpts_decimal", 0)
        fpts_total = fpts + fpts_dec / 100
    ...
    df["Win%"] = df["W"] / (df["W"] + df["L"]).replace(0, 1)
    df = df.sort_values(["Win%", "PF"], ascending=[False, False]).reset_index(drop=True)
    df.index += 1
    return df

# Funktion: letztes Matchup abrufen
def get_last_matchups(league_id):
    league = League(league_id)
    current_week = league.get_league()["settings"]["playoff_week_start"]
    week = current_week - 1 if current_week > 1 else 1  # falls erste Woche
    ...
    for m in matchups:
        matchup_df.append({
            "manager": roster_map[m["roster_id"]],
            "points": m.get("points", 0),
            "matchup_id": m["matchup_id"]
        })

    df = pd.DataFrame(matchup_df)
    # Matchups paarweise zusammenführen
    merged = (
        df.groupby("matchup_id")
          .apply(lambda g: {
              "Team 1": g.iloc[0]["manager"],
              "Pts 1": g.iloc[0]["points"],
              "Team 2": g.iloc[1]["manager"] if len(g) > 1 else None,
              "Pts 2": g.iloc[1]["points"] if len(g) > 1 else None,
          })
          .apply(pd.Series)
...
# Tabs pro Liga
for league_id in REDRAFTLEAGUES_2025:
    league = League(league_id)
    ...
    with st.expander(f"🏈 {league_name}"):
        ...
        with col2:
            st.subheader("Letzte Matchups")
            matchups_df = get_last_matchups(league_id)
