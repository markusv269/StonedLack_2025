...
#     """Sendet Anmeldeschlüssel per E-Mail via SendGrid"""
...
#         st.success(f"✅ Bestätigung wurde an {to_email} gesendet.")
#     else:
...
#         sendgrid_mail(email, display_name, schluessel)
...
#         email = st.text_input("Deine E-Mail-Adresse (für Schlüssel & Bestätigung)", key="email")
#         schluessel_input = st.text_input("🔐 Anmeldeschlüssel (nur bei Änderung)", help="Nur erforderlich, wenn du deine Anmeldung ändern willst.")
#         submitted = st.form_submit_button("Anmeldung absenden")

#         if submitted:
#             if not sleeper_name or not discord_name or not email:
